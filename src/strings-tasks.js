...
//   let i = 0;
//   while (i < str.length) {
//     str2 = str2.padStart(str2.length + 1, str.at(i));
//     i += 1;
//   }
//   return str2;
// }
...
function isPalindrome(str) {
  ...
  if (
    str
      .replaceAll(/[, !?]/g, '')
      .localeCompare(str2.replaceAll(/[, !?]/g, ''), 'en', {
        sensitivity: 'base',
      }) === 0
  ) {
    return true;
...
// return Boolean( str.replaceAll(/[, !?]/g, '') === str.replaceAll(/[, !?]/g, '').split("").reverse().join(""));
...
function invertCase(str) {
  function changeEach(x) {
    const lowCse = /[a-z]/;
    const upCse = /[A-Z]/;
    if (upCse.test(x) === true) {
      return x.toLowerCase();
    }
    if (lowCse.test(x) === true) {
      return x.toUpperCase();
    }
    return x;
  }
  return str.split('').map(changeEach).join('');
}
...
function encodeToRot13(str) {
  function changeEach(x) {
    const firstPart = /[A-M]/i;
    const secondPart = /[N-Z]/i;
    if (firstPart.test(x) === true) {
      return String.fromCharCode(x.charCodeAt(0) + 13);
    }
    if (secondPart.test(x) === true) {
      return String.fromCharCode(x.charCodeAt(0) - 13);
    }
    return x;
  }
